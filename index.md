...
Next, we change the the layers in which the style loss function is computed for. Previously, we used the first five convolutional layers of the VGG-19 network in the style loss. We compare that to using the first convolutional layer and the fifth convolution layer in the style loss function. We can see that when using only the first layer, the generated image is mostly just tinted blue, so we can guess that the first convolutional layer in the network mostly detects blue colors. If we only use the fifth layer, the generated image contains a lot of wavey brushstrokes, so we can guess that the fifth convolutional layer detects a lot of wavey patterns.
